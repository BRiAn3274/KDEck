...
- 从 Steam Deck 发送文件到手机：在 Decky 侧边栏浏览截图、录像和日志，支持搜索、进度动画和发送前连接验证
...
2. 选择"发送文件"进入文件管理页面。
3. 选择分类（截图/录像/日志），可以用搜索框按文件名过滤。
4. 如果有多个可用设备，先选择目标设备，再选择要发送的文件。
5. 插件会在发送前验证设备连接状态，传输过程中显示进度动画。
...
KDEck 正持续开发中。剪贴板接收、文件接收和文件发送（截图、录像、日志）已经过多款手机实机验证。自动发现在大多数家用 Wi-Fi 下正常工作；受限或多网卡环境下仍可手动添加 Deck IP 作为兜底。
