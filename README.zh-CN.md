...
KDEck 是一个给 Steam Deck 游戏模式使用的 Decky Loader 插件。它让 KDE Connect 设备可以把文本和文件发送到 Steam Deck，不需要打开 KDE 桌面模式。
...
- KDE Connect 设备发送剪贴板文本，KDEck 会显示在插件的一行文本框里
- KDE Connect 设备发送文件，KDEck 会保存到 Steam Deck 的 `Downloads` 目录
- 从 Steam Deck 发送文件到 KDE Connect 设备：浏览截图、录像、日志和存档，支持搜索、排序、过滤、缩略图、真实进度、速率和预计剩余时间
- 在日志页直接发送脱敏诊断包
- 插件内显示 Deck 当前可用 IP，可以为手机手动添加设备做兜底
- 插件内显示最近收到的文件
- 通过 `@decky/ui` TextField 实现原生屏幕键盘支持，QAM 面板和收发文件页面均可正常唤起

KDEck 目前聚焦于双向剪贴板文本和文件传输，未做通知、短信、远程输入、媒体控制等完整 KDE Connect 桌面功能。
...
KDEck 会保留自己的 KDE Connect 设备身份、证书和可信设备记录。覆盖安装或卸载后重新安装时，已配对的手机或电脑通常不需要取消配对再重新配对。如果你想主动清空身份和配对记录，可以在隐藏命令中使用 `:kdeck reset identity`，然后重启 KDEck 并重新配对。
...
从 Deck 发送文件到 KDE Connect 设备：

1. 打开 Decky 侧边栏的 KDEck 面板。
2. 选择"发送文件"进入文件管理页面。
3. 选择分类（截图/录像/日志/存档），可以搜索、排序或过滤列表。
4. 如果有多个可用设备，先选择目标设备，再选择要发送的文件。
5. KDEck 会显示当前传输阶段、已发送字节、速率和预计剩余时间。

发送诊断包：

1. 打开“发送文件”，切换到“日志”页面。
2. 选择“发送诊断包”。
3. KDEck 会导出现有脱敏日志 zip，并通过同一套可追踪发送流程发给手机或电脑。
...
如果你希望从 KDEck 侧彻底重置身份，可以在 KDEck 文本框输入 `:kdeck reset identity` 并按 Enter。该命令会删除 KDEck 的设备 ID、证书、私钥、可信设备记录和首选发送设备；执行后需要重启 KDEck 并重新配对。
...
KDEck 正持续开发中。剪贴板接收、文件接收和文件发送（截图、录像、日志、存档）已经过多款手机实机验证。自动发现在大多数家用 Wi-Fi 下正常工作；受限或多网卡环境下仍可手动添加 Deck IP 作为兜底。
...
KDEck 是独立项目，不隶属于 KDE e.V. 或 KDE Connect 项目，也不代表 KDE 官方发布。KDEck 只实现双向剪贴板文本和文件传输所需的最小兼容接收端，不包含 KDE Connect 源码，也不提供完整 KDE Connect 功能集。
...
KDEck 后端会记录 receiver 是否期望运行、是否因为桌面模式暂停、UDP/TCP 是否监听、最近 UDP discovery 来源、最近 TCP 成功/失败、最近 TLS 成功/失败、最近配对状态、可信设备 reannounce 目标、最近剪贴板状态、最近文件状态、发送任务状态、最近 payload 传输错误和连接状态变化。事件日志会统一包含 `event`、`stage`、`device_id`、`time`，同类高频错误会限流，路径、命令、fingerprint 等敏感字段会脱敏。日志导出包里的 `manifest.json` 和 `status-snapshot.json` 会包含脱敏诊断信息，方便判断问题卡在发现、连接、TLS、配对还是文件传输阶段。
...
在 KDEck 剪贴板文本框输入其中一条并按 Enter，或者双击"同步文本框"按钮。隐藏命令的意义是：普通用户仍把这个文本框当作剪贴板展示框，测试者可以把它当作一个很小的诊断控制台。命令文本不会作为剪贴板内容保存。
...
- `:kdeck reset identity` 会清除 KDEck 的 managed KDE Connect 身份、证书、可信设备记录和首选发送设备。只在需要彻底重新配对时使用。
- `:kdeck update <https-url> <sha256>` 会下载安装开发更新包。URL 必须使用 HTTPS，SHA256 必须和下载的 zip 一致。
